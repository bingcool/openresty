...
do
    if ngx then
        ...
        local function v3_hash(binary, name)
            ...
            return hash, tohex(bor(band(hex_to_i(sub(hash, 13, 14)), 0x0F), 0x30), 2), tohex(
                bor(band(hex_to_i(sub(hash, 17, 18)), 0x3F), 0x80),
                2
            )
        end
        ...
        local function v5_hash(binary, name)
            ...
            return hash, tohex(bor(band(hex_to_i(sub(hash, 13, 14)), 0x0F), 0x50), 2), tohex(
                bor(band(hex_to_i(sub(hash, 17, 18)), 0x3F), 0x80),
                2
            )
        end
